...
formatting_rules_file="${state_dir}/formatting_rules.json"
...
if [[ ! -f "${formatting_rules_file}" ]]; then
  echo "ERROR: missing ${formatting_rules_file}" >&2
  exit 1
fi
...
jq --indent 2 --slurp '{
  ...
  llm_connect: .[1],
  formatting_rules: .[2]
}' "${settings_file}" "${llm_connect_file}" "${formatting_rules_file}" >"${config_file}"
