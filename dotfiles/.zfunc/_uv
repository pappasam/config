...
'*--index=[The indexes to use when resolving dependencies, in addition to the default index]:INDEX:_default' \
'--default-index=[The default package index (by default\: <https\://pypi.org/simple>)]:DEFAULT_INDEX:_default' \
...
'*--prerelease-package=[The strategy to use when considering pre-release versions for a specific package]:PRERELEASE_PACKAGE:' \
...
'(--check-exists --check --locked --dry-run)--frozen[Equivalent to \`--check-exists\`]' \
'--no-frozen[Disable frozen mode, overriding \`UV_FROZEN\`]' \
...
(upgrade)
_arguments "${_arguments_options[@]}" : \
'*--exclude=[Exclude the named package from upgrades]:EXCLUDE:' \
'--cache-dir=[Path to the cache directory]:CACHE_DIR:_files -/' \
'--python-preference=[]:PYTHON_PREFERENCE:((only-managed\:"Only use managed Python installations; never use system Python installations"
managed\:"Prefer managed Python installations over system Python installations"
system\:"Prefer system Python installations over managed Python installations"
...
'--exclude-newer=[Limit candidate Ruff versions to those released prior to the given date]:EXCLUDE_NEWER:' \
'--cache-dir=[Path to the cache directory]:CACHE_DIR:_files -/' \
'--python-preference=[]:PYTHON_PREFERENCE:((only-managed\:"Only use managed Python installations; never use system Python installations"
managed\:"Prefer managed Python installations over system Python installations"
system\:"Prefer system Python installations over managed Python installations"
...
(check)
_arguments "${_arguments_options[@]}" : \
'(--all-packages --script --no-project)*--package=[Check specific packages in the workspace]:PACKAGE:' \
'(--extra --all-extras --no-extra --no-all-extras --dev --no-dev --only-dev --group --no-group --no-default-groups --only-group --all-groups --no-project --all-packages --package)--script=[Run checks for the specified PEP 723 Python script, rather than the current project]:SCRIPT:_files' \
'(--all-extras --only-group)*--extra=[Include optional dependencies from the specified extra name]:EXTRA:' \
'*--no-extra=[Exclude the specified optional dependencies, if \`--all-extras\` is supplied]:NO_EXTRA:' \
'(--only-group --only-dev)*--group=[Include dependencies from the specified dependency group]:GROUP:' \
'*--no-group=[Disable the specified dependency group \[env\: \`UV_NO_GROUP\`=\]]:NO_GROUP:' \
...
'(--package --script --no-project)--all-packages[Check all packages in the workspace]' \
'(--extra --only-group)--all-extras[Include all optional dependencies]' \
'--no-all-extras[]' \
'--dev[Include the development dependency group \[env\: UV_DEV=\]]' \
'--no-dev[Disable the development dependency group \[env\: UV_NO_DEV=\]]' \
...
'--no-frozen[Disable frozen mode, overriding \`UV_FROZEN\`]' \
'--no-sync[Avoid syncing the virtual environment \[env\: UV_NO_SYNC=\]]' \
'(--no-sync --script --no-project)--no-install-project[Do not install the current project \[env\: UV_NO_INSTALL_PROJECT=\]]' \
...
'-h[Display the concise help for this command]' \
'--help[Display the concise help for this command]' \
'*::name -- The names of the installed tools to audit:' \
&& ret=0
;;
(uninstall)
_arguments "${_arguments_options[@]}" : \
...
(pip)
_arguments "${_arguments_options[@]}" : \
'--cert=[Path to a PEM-encoded CA certificate bundle]:FILE:_files' \
'--cache-dir=[Path to the cache directory]:CACHE_DIR:_files -/' \
'--python-preference=[]:PYTHON_PREFERENCE:((only-managed\:"Only use managed Python installations; never use system Python installations"
managed\:"Prefer managed Python installations over system Python installations"
system\:"Prefer system Python installations over managed Python installations"
...
(size)
_arguments "${_arguments_options[@]}" : \
'--output-format=[Select the output format]:OUTPUT_FORMAT:((auto\:"Display a human-readable size in terminals and raw bytes otherwise"
...
'--project=[Discover a project in the given directory]:PROJECT:_files -/' \
'--config-file=[The path to a \`uv.toml\` file to use for configuration]:CONFIG_FILE:_files' \
'(--output-format)-H[Display the cache size in human-readable format (e.g., \`1.2GiB\` instead of raw bytes)]' \
'(--output-format)--human[Display the cache size in human-readable format (e.g., \`1.2GiB\` instead of raw bytes)]' \
'-n[Avoid reading from or writing to the cache, instead using a temporary directory for the duration of the operation]' \
'--no-cache[Avoid reading from or writing to the cache, instead using a temporary directory for the duration of the operation]' \
'--managed-python[Require use of uv-managed Python versions \[env\: UV_MANAGED_PYTHON=\]]' \
'--no-managed-python[Disable use of uv-managed Python versions \[env\: UV_NO_MANAGED_PYTHON=\]]' \
...
'audit:Audit installed tools and their dependencies' \
...
(( $+functions[_uv__tool__audit_commands] )) ||
_uv__tool__audit_commands() {
    local commands; commands=()
    _describe -t commands 'uv tool audit commands' commands "$@"
}
